...
- [x] 仓库书签页面（添加/删除/打开仓库）
- [x] 打开本地仓库（gix::Repository::open）
- [x] 仓库主页布局（三栏：侧边栏 + 文件列表 + 差异面板）
- [x] 文件状态视图（未暂存/已暂存/未跟踪/忽略）
- [x] 基础差异查看（unified diff 视图）
- [x] 暂存操作（stage/unstage 文件）
- [x] 提交功能（填写信息 + commit）
- [x] 仓库信息面板（当前分支、远程地址等）
...
- [x] 创建/切换/删除分支
- [x] 分支列表（本地/远程分组）
- [x] Fetch / Pull / Push 操作
- [x] 领先/落后远程分支指示
- [x] 远程仓库管理（添加/编辑/删除 remote）
- [x] 克隆仓库功能
- [x] 初始化仓库功能
...
- [ ] 冲突解决界面（三栏对比 + 手动编辑）
- [ ] Stash 创建/应用/弹出/删除
- [x] 标签创建/删除/推送（创建：在HistoryPage中右键添加标签创建按钮）
...
| 阶段 | 状态 | 关键交付 |
|------|------|----------|
| 阶段 0：项目基础搭建 | **已完成 (100%)** | 可运行空壳应用，前后端通信正常 |
| 阶段 1：仓库管理核心 | **已完成 (100%)** | 打开仓库 + 提交 |
| 阶段 2：提交历史与分支 | **已完成 (100%)** | DAG 图 + 分支管理 |
