...
import { openRepo, getCurrentBranch, cloneRepo, initRepo } from "@/services/git";
...
export default function WelcomePage() {
  ...
  const handleClone = async () => {
    ...
    setErrMsg("");
    try {
      await cloneRepo(cloneUrl.trim(), cloneDir.trim());
      // 添加到书签并打开
  ...
  const handleInit = async () => {
    ...
    setErrMsg("");
    try {
      await initRepo(initPath.trim(), false);
      // 添加到书签并打开
          ...
          <div className="mb-6 p-4 rounded-lg border border-border bg-card">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <button
                  onClick={handleSelectCloneDir}
                  className="px-3 py-2 text-xs rounded border border-input hover:bg-accent"
                >
                  浏览
                </button>
              </div>
